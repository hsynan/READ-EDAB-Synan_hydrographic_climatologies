...
@title: hydro_climas_func_processraw 
@author: haley.synan
@category:
    FUNCTIONS 
@purpose: 
    Functions to process the raw data to create the hydrographic climatologies
History: 
...
def avg_bot_vert(indata, data_source):
    ...
    float_cols = ['temperature', 'salinity', 'SA', 'CT', 'pressure','density']
    ...
    # Calculate stats 
    profile_stats = filtered_df.groupby('profile_uid').agg(
        temp_min=('temperature', 'min'),
        temp_max=('temperature', 'max'),
        sal_min=('salinity', 'min'),
        sal_max=('salinity', 'max'),
        num_obs=('pressure', 'count'), # Count from filtered_df instead? Use separate agg
        pres_min= ('pressure','min'),
        pres_max=('pressure','max')
    )
    profile_stats['temp_std']=filtered_df.groupby('profile_uid')['temperature'].std()
    profile_stats['sal_std']=filtered_df.groupby('profile_uid')['salinity'].std()
    # put back together 
...
def gsw_conversions(df):
    ...
    df['density'] = gsw.rho(df.SA.astype(float), df.CT.astype(float), df.pressure.astype(float))
