...
@title: hydro_climas_exec_processraw_BOTTOM
@author: haley.synan
@category:
    Exectuable
@purpose: 
    Exectuable script to process the raw data to create the hydrographic climatologies for the bottom
History: 
...
if loc=='local': #this will need to change
    base_dir=os.path.join('PROJECT_DIR','hydro_clima')
else: 
    base_dir = os.path.join('PROJECT_DIR','hydro_clima')

# load trees and source dictionary
source = get_source(local_dir=os.path.join(base_dir,'SOURCE_DATA')) 
url =requests.get('https://hfr.marine.rutgers.edu/erddap/griddap/bathymetry_srtm15_v24.nc?z%5B(33):1:(47)%5D%5B(-78):1:(-62)%5D',verify=False).content
bathy = xr.open_dataset(url)
b=bathy.to_dataframe().reset_index()
b = b[b.z<0].reset_index() #get rid of positive vlaeus
flat_coords = np.column_stack((b.latitude, b.longitude))
bathy_tree = cKDTree(flat_coords)



file_dir=os.path.join(base_dir,'PROCESSED_DATA','POINT_MEAN_BOTTOM')
...
if source[data_source]['filetype'] == 'csv':
    for fname in source[data_source]['urls']:
        #LOAD DATA
        if (source[data_source]['name']=='dfo') or (source[data_source]['name']=='dfo_buoys'):
            ...
            if parsed.scheme in ('http', 'https', 'erddap'):
                print(f"Reading from URL/ERDDAP: {fname}")
                df = pd.read_csv(fname)
                
            # Otherwise, it's a local file or directory path
            else:
                print(f"Reading from local source: {fname}")
                df = pd.read_csv(fname)
                df = df[1:]
            
           # if (isinstance(df.latitude.iloc[0], str) & isinstance(df.time.iloc[0], str) & isinstance(df.pressure.iloc[0], str)) == True:
            #if all(isinstance(val, str) for val in df.iloc[0,:4]) == True:
            #if df.iloc[0].apply(lambda x: isinstance(x, str) or pd.isna(x)).all() == True:
            #    df =df[1:]
            #elif (isinstance(df[df.filter(like='latitude').columns[0]].iloc[0], str) & isinstance(df.time.iloc[0], str)) == True:
            #    df =df[1:]
        #PREPROCESSING
        ...
        #
        if source[data_source]['name']=='pam':
            df['salinity'] = 2.222222 #create dummy salinity data so it can go thru the pipeline without breaking
        if source[data_source]['name'] == 'fishbot':
            df['salinity'] = df['salinity'].fillna(2.222222) #fill nas with dummy data so it can go thru pipeline without data loss  (is filtered out before salinity interpolation)
        ...
        if df is None:
            ...
            if source[data_source]['profiletype']=='vertical':
                ...
                print('processing as vertical profile...')
                ...
                df = match_bathy(df,b,bathy_tree)
                proc = avg_bot_vert(df,data_source)
                new_fname = 'PROCESSED_MEAN_BOTTOM_'+data_source.upper()+'_'+dataset_id+'.csv'
                ...
                proc = avg_mld_buoy_bot(df)
                if source[data_source]['name']=='pam':
                    proc['salinity'] = np.nan
                    proc['SA'] = np.nan
                    proc['min_prof_sal'] = np.nan
                    proc['max_prof_sal'] = np.nan
                new_fname = 'PROCESSED_MEAN_BOTTOM_'+data_source.upper()+'_'+dataset_id+'.csv'
                proc.to_csv(os.path.join(file_dir,new_fname))
                print(f'Processing {data_source} as buoys')
            elif source[data_source]['profiletype'] == 'agg':
                 print(f'Processing {data_source} as aggregated bottom product')
                 new_fname = 'PROCESSED_MEAN_BOTTOM_'+data_source.upper()+'_'+dataset_id+'.csv'
                 df.to_csv(os.path.join(file_dir,new_fname))
        ...
        try: 
            ...
            if {'temperature','salinity'}.issubset(df.columns)==False:
                ...
                if 'time' not in df.columns:
                    ...
                    df = match_bathy(df,b,bathy_tree)
                    proc=avg_bot_vert(df, data_source)
                    new_fname = 'PROCESSED_MEAN_ABOVEMLD_BOTTOM_'+data_source.upper()+'_'+dataset_id+'.csv'
                    proc.to_csv(os.path.join(file_dir,new_fname))
                    # print(f'Processing for {source[data_source]['name']} complete')
        except ValueError:
