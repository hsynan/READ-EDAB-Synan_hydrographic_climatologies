...
@title: hydro_climas_exec_processraw 
@author: haley.synan
@category:
    Exectuable
@purpose: 
    Exectuable script to process the raw data to create the hydrographic climatologies for the surface
History: 
        ...
        if (source[data_source]['name']=='dfo') or (source[data_source]['name']=='dfo_buoys'):
           ...
           # if (isinstance(df.latitude.iloc[0], str) & isinstance(df.time.iloc[0], str) & isinstance(df.pressure.iloc[0], str)) == True:
            ...
            if df.iloc[0].apply(lambda x: isinstance(x, str) or pd.isna(x)).all() == True:
                df =df[1:]
            elif (isinstance(df.latitude.iloc[0], str) & isinstance(df.time.iloc[0], str)) == True:
                df =df[1:]
            #if source[data_source] =='argo':
            #    df=df[1:]
            #    print(df.iloc[:4])
